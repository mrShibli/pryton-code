...
class UserController extends Controller
{
  ...
  public function profile($slug, $media = null)
  {
    ...
    //=== about
    $aboutUser = null; // Initialize the variable outside the closure

    $query->when(request('media') == 'about', function ($q) use ($user, &$aboutUser) {
      $aboutUser = User::find($user->id);

      if ($aboutUser) {
        $aboutUser = $aboutUser->story;
      }
    });

    // Now $aboutUser is accessible outside the closure
    $plans =  DB::table('plans')->where('user_id', $user->id)
      ->where('status', '1')
      ->get();
  ...
  public function updateSettingsPage()
  {
    ...
    if ($this->request->hasFile('review_img')) {
      $image = $this->request->file('review_img');
      $imageName = Carbon::now()->timestamp . '_' . uniqid() . '.' . $image->getClientOriginalExtension();
      $image->move(public_path('img/review'), $imageName);
      $user                  = User::find($id);
      $user->review_img = $imageName;
      $user->save();
    }
  ...
  {
    // $tableName = 'plans'; // Replace with your actual table name
    ...
    if (!$this->request->free_subscription) {
      ...
      $validator = Validator::make(request()->all(), [
        'price_one' => 'required|numeric|min:' . $currencyPosition . $this->settings->min_subscription_amount . '|max:' . $this->settings->max_subscription_amount . '',
        'two_price' => 'required|numeric|min:' . $currencyPosition . $this->settings->min_subscription_amount . '|max:' . $this->settings->max_subscription_amount . '',
        'price_three' => 'required|numeric|min:' . $currencyPosition . $this->settings->min_subscription_amount . '|max:' . $this->settings->max_subscription_amount . '',
        'one_title' => 'required',
        'two_title' => 'required',
      ...
      // dd($this->request->two_title); $this->request->free_subscription
      ...
      if ($this->request->price_one) {
        // Save the image file
        if ($this->request->hasFile('one_img')) {
          $file = $this->request->file('one_img');

          // Generate a unique filename
          $filename = 'image_one_' . now()->format('YmdHis') . '_' . Str::random(10) . '.' . $file->getClientOriginalExtension();

          // Save the file to the public folder
          $file->storeAs('public/images', $filename);

          // You can also store the filename in the database or perform any other necessary operations
          // For example, you might want to save $filename in the corresponding model field
          // $yourModel->update(['img' => $filename]);
        }

        // Update or create the plan DB::table('plans')->updateOrInsert
        // DB::table('plans')
        $plan = DB::table('plans')->updateOrInsert(
          [
            'user_id' => auth()->id(),
            'name' => 'user_' . auth()->id() . '_price_one',
          ],
          [
            'price' => $this->request->price_one,
            'interval' => 'monthly',
      ...
      // dd($this->request->all());
      // Subscription Price (2 months)
      if ($this->request->two_price) {
        // Save the image file
        if ($this->request->hasFile('two_img')) {
          $file = $this->request->file('two_img');

          $filename = 'two_img_' . now()->format('YmdHis') . '_' . Str::random(10) . '.' . $file->getClientOriginalExtension();
        ...
        $plan = DB::table('plans')->updateOrInsert(
          [
            'user_id' => auth()->id(),
            'name' => 'user_' . auth()->id() . '_price_two',
          ],
          [
            'price' => $this->request->two_price,
            'interval' => 'monthly',
      ...
      if ($this->request->price_three) {
        // Save the image file
        if ($this->request->hasFile('image_three')) {
          $file = $this->request->file('image_three');

          // Generate a unique filename
          $filename = 'image_three_' . now()->format('YmdHis') . '_' . Str::random(10) . '.' . $file->getClientOriginalExtension();
        ...
        $plan = DB::table('plans')->updateOrInsert(
          [
            'user_id' => auth()->id(),
            'name' => 'user_' . auth()->id() . '_price_three',
          ],
          [
            'price' => $this->request->price_three,
            'interval' => 'monthly',
