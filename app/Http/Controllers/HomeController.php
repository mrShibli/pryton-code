...
class HomeController extends Controller
{
  ...
  public function creators($type = false)
  {
    ...
    if ($this->request->has('country')) {
      $countryId = $this->request->country;
  
      $users = User::where('countries_id', $countryId)->paginate(10);
  
      // Check if any user in the paginated results has the specified 'countries_id'
      $hasCountryId = $users->contains('countries_id', $countryId);
  ...
  //     $title = __('general.featured_creators');
  //     $countryId = $this->request->country;
  //     $users = User::where('countries_id', $countryId)->paginate(10); // Adjust the number per page as needed
