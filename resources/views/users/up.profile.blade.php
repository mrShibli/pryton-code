...
<!-- avatar-wrap -->
                        ...
                        <div class="">
                            <div class="row">
                                ...
                                <div class="col-xs-12 col-sm-4 col-lg-4 mx-auto">
                                    <div class="pricingTable rounded pricingTable-2 border p-0" {!! isset($key) && $key==1 ? 'style="border-top: 0px !important;"' : '' !!}>
                                        @if(isset($key) && $key == 1)
                                            <p class="m-0" style="border-top-right-radius:15px;border-top-left-radius:15px; text-align: center;background-color: #7f28b5; color:white;">
                                                YOU MIGHT LIKE
                                            </p>
                                        @endif
                                        <img class="p-0 img-fluid" src="{{ asset('/public/images/'.$plan->img) }}" alt="" srcset="" style="padding: 0px !important;">
                                        <div class="pricingTable-header px-3">
                                            <p class="text-left ml-3" style="font-size:22px;">{{ $plan->title }}</p>
                                            <div class="text-left ml-3">
                                                <span class="font-weight-bold" style="font-size:19px;">{{ $plan->price }}</span>/
                                                <span class=""><span class="value-bg"></span>{{ $plan->interval }}</span>
                                            </div>
                                            <a href="javascript:void(0);" data-toggle="modal" data-target="#subscriptionForm{{$plan->id}}" class="btn btn-lg text-white" style="background-color: #7f28b5; width: 100%;">
                                                <i class="feather icon-unlock mr-1"></i> Join {{__('', ['price' => $plan->price])}}
                                            </a>
                                        </div>
            ...
            @foreach ($plans as $plan)
                <div class="modal fade" id="subscriptionForm{{$plan->id}}" tabindex="-1" role="dialog" aria-labelledby="modal-form{{$plan->id}}" aria-hidden="true">
                                    ...
                                    {{-- <div class="card-body px-lg-5 py-lg-5 position-relative">
                                        <div class="text-muted text-center mb-3 position-relative modal-offset">
                                            ...
                                            <h6 class="font-weight-light">
                                                {!! __('general.subscribe_month', ['price' => '<span class="font-weight-bold">'.$user->getPlan($plan->id,'monthly', 'price'), true.'</span>']) !!} {{__('general.unlocked_content')}}
                                                {{$user->hide_name == 'yes' ? $user->username : $user->name}}
                                        ...
                                        <form method="post" action="{{url('buy/subscription')}}" id="formSubscription">
                                            ...
                                            <input name="interval" value="monthly" id="plan-monthly{{$plan->id}}" class="d-none" type="radio">
                                            <input name="interval" value="{{ $plan->interval }}" id="plan-{{ $plan->interval }}{{$plan->id}}" class="d-none" type="radio">
        
                                            @foreach ($allPayment as $payment)
                                                <!-- Loop through your payment methods as before -->
                                            @endforeach
        
                                            @if (($settings->disable_wallet == 'on' && Helper::userWallet('balance') != 0) || $settings->disable_wallet == 'off')
                                            <!-- Wallet information as before -->
                                            @endif
        
                                            <!-- Rest of the form content as before -->
                                        </form>
                                    </div> --}}
